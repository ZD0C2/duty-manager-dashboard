...
-- ---------------------------------------------------------------------------
-- Follow-up: staff-created room QRs and verified hotel directory identities
-- ---------------------------------------------------------------------------
...
begin
  if not exists (
    ...
    alter table public.properties add constraint properties_external_place_pair_check
      check (
        (external_place_provider is null and external_place_id is null)
        or (external_place_provider = 'google_places' and char_length(external_place_id) between 3 and 255)
      );
  end if;
end $$;
...
on public.property_locations for insert to authenticated
with check (location_type = 'room' and private.is_property_staff(property_id));
